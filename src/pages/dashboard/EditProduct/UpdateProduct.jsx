...
const UpdateProduct = () => {
  ...
  const onSubmit = async (data) => {
    ...
    // ✅ Assurer que seules les catégories valides sont enregistrées
...
return (
  <div className="max-w-lg mx-auto p-6 bg-white rounded-lg shadow-md">
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <label className="font-semibold text-gray-700">Quantité en Stock</label>
      ...
      {previewURL && <img src={previewURL} alt="Aperçu" className="w-64 h-64 object-cover border rounded-md mt-4 mx-auto" />}

      <label className="font-semibold text-gray-700">Couleurs</label>
      {colors.map((color, index) => (
        <div key={index} className="space-y-2 border p-3 rounded-md">
          <input type="text" value={color.colorName} onChange={(e) => handleColorChange(index, "colorName", e.target.value)} className="w-full p-2 border rounded" placeholder="Nom de la Couleur" required />
          ...
          {color.previewURL && <img src={color.previewURL} alt="Aperçu de la Couleur" className="w-20 h-20 mt-2" />}
          <button type="button" onClick={() => deleteColor(index)} className="px-3 py-1 bg-red-500 text-white rounded">Supprimer la Couleur</button>
        </div>
      ))}
      <button type="button" onClick={addColor} className="px-3 py-2 bg-gray-300 rounded">Ajouter une Couleur</button>
